...
class Encoder(nn.Module):
  def __init__(self, in_channels, feature_dim):
    ...
    self.layers = nn.Sequential(
        nn.Dropout(p=0.5),
        nn.Conv2d(in_channels=in_channels, out_channels=32, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(num_features=32, eps=1e-5, momentum=0.99), nn.ReLU(),
        nn.MaxPool2d(kernel_size=4, stride=2, padding=1), nn.ReLU(), nn.Dropout(p=0.5),
        nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(num_features=64, eps=1e-5, momentum=0.99), nn.ReLU(),
        nn.MaxPool2d(kernel_size=4, stride=2, padding=1), nn.ReLU(), nn.Dropout(p=0.5),
        nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(num_features=64, eps=1e-5, momentum=0.99), nn.ReLU(),
        Flatten(),
        nn.Linear(in_features=16384, out_features=feature_dim), nn.ReLU()
    )
...
while step < total_steps:
  ...
  if(step % 999424 == 0): # we save every 1e6 ish timesteps
    torch.save(policy.state_dict(), 'checkpoints/baseline_advModel.pt')
    torch.save(total_training_reward, 'trainingResults/training_Reward_advModel.pt')
